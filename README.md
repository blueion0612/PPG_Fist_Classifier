...
[[Method]](#method) · [[Results]](#results) · [[Real-time inference]](#real-time-inference) · [[Data]](#data)

Detects whether a hand is open or clenched from the photoplethysmography sensor
already present in a wrist device. No extra hardware, no electrodes: a fist
squeezes the tissue under the sensor and changes the perfusion signal, and that
change is what the classifier reads.

The repository holds the full chain from raw recordings to a real-time UDP
service, plus the experiment that produced the numbers below.
...
instead trains a multi-scale CNN (kernels 3, 5 and 7) on raw windows from the
three channels that separate the classes best on their own: ch01, ch05 and ch07,
all with Cohen's *d* above 0.5.

## Results

Seven recording sessions, three with a normal strap and four with the strap
pulled tight. Every number below comes from the multi-scale CNN, averaged over
sessions. Regenerate the whole set with `python scripts/report_experiment.py`.
...
the class ordering. The result is kept in the table because it rules the approach
out.

Confusion matrix, ROC curve, within-session breakdown and the calibration sweep are
in [`docs/figures/`](docs/figures).

**Read the calibration numbers with care.** The calibration window is taken from
the head of the held-out session, so a longer calibration leaves a smaller and
later test set. The drop from 20 s to 30 s is therefore not evidence that more
calibration hurts. The 40 s and 60 s settings in the sweep produced no result at
all, because no session was long enough to leave a usable test split.
...
docs/figures/            the six result figures
